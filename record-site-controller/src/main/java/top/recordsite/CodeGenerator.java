...
public class CodeGenerator {
    public static void main(String[] args) {
        ...
        globalConfig.setOutputDir(System.getProperty("user.dir")+"/record-site-controller/src/main/java/");    //设置代码生成位置
...
//        globalConfig.setIdType(IdType.AUTO);   //设置Id生成策略
        ...
        packageInfo.setEntity("entity.pojo");    //设置实体类包名
        packageInfo.setMapper("mapper");   //设置数据层包名
        autoGenerator.setPackageInfo(packageInfo);

        //5.策略设置
        StrategyConfig strategyConfig = new StrategyConfig();
        strategyConfig.setInclude("acg_anime_user");  //设置当前参与生成的表名，参数为可变参数
        strategyConfig.setTablePrefix("acg_");  //设置数据库表的前缀名称，模块名 = 数据库表名 - 前缀名  例如： User = tbl_user - tbl_
